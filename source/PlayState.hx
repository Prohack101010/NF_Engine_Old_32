...
class PlayState extends MusicBeatState
{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				notes.forEachAlive(function(note:Note) {
					if(ClientPrefs.opponentStrums || !ClientPrefs.opponentStrums && ClientPrefs.betterMidScroll || note.mustPress)
					{
						...
						if(ClientPrefs.betterMidScroll && !note.mustPress) {
							note.alpha *= 0.35;
						}
					}
				});
	...
	private function generateSong(dataPath:String):Void
	{
		...
		#if MODS_ALLOWED
		if (FileSystem.exists(Paths.modsJson(songName + '/events')) || FileSystem.exists(SUtil.getStorageDirectory() s+ file)) {
		#else
		if (OpenFlAssets.exists(file)) {
		#end
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						else if(ClientPrefs.betterMidScroll)
						{
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player < 1)
			{
				...
				else if(ClientPrefs.middleScroll || ClientPrefs.betterMidScroll) targetAlpha = ClientPrefs.oppNoteAlpha;
				if (ClientPrefs.betterMidScroll) opponentStrums.members[i].x == FlxG.width - 2; //make it so that you're unable to see the opponent's strums if you have better styled middlescroll on
			}

			var babyArrow:StrumNote = new StrumNote(ClientPrefs.middleScroll || ClientPrefs.betterMidScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player);
			...
			if (player == 1)
			{
				if (!opponentChart || opponentChart && ClientPrefs.middleScroll || opponentChart && ClientPrefs.betterMidScroll) playerStrums.add(babyArrow);
			...
			else
			{
				...
				if(ClientPrefs.betterMidScroll)
				{
				...
				if (!opponentChart || opponentChart && ClientPrefs.betterMidScroll) opponentStrums.add(babyArrow);
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if (!cpuControlled) // bro this is not cool
